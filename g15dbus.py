...
class G15DBUSScreenService(AbstractG15DBUSService):
    def __init__(self, dbus_service, screen):
        ...
        self._notify_handles.append(
            self._screen.conf_client.notify_add("/apps/gnome15/%s/cycle_screens" % self._screen.device.uid,
    ...
    def AcquireControl(self, control_id, release_after, value, sender=None):
        ...
        control_acquisition = self._screen.driver.acquire_control(control,
                                                                  None if release_after == 0 else release_after,
                                                                  initial_value)
        dbus_control_acquisition = G15DBUSControlAcquisition(self, control_acquisition,
                                                             self._dbus_service._acquire_sequence_number)
    ...
    def GetDeviceInformation(self):
        ...
        return (
        device.uid, device.model_id, "%s:%s" % ( hex(device.controls_usb_id[0]), hex(device.controls_usb_id[1]) ),
        device.model_fullname )
    ...
    def GetDriverInformation(self):
        ...
        return (driver.get_name(), driver.get_model_name(), driver.get_size()[0], driver.get_size()[1],
                driver.get_bpp() ) if driver != None else None
...
class G15DBUSService(AbstractG15DBUSService):
    def __init__(self, service):
        ...
        self._bus_name = dbus.service.BusName(BUS_NAME, bus=self._bus, replace_existing=False, allow_replacement=False,
                                              do_not_queue=True)
