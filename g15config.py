...
try:
    ...
    logger.debug('Could not load appindicator module %s' % str(e))
    pass
...
class G15ConfigService(dbus.service.Object):
    ...
    def __init__(self, config):
        self._config = config
        bus_name = dbus.service.BusName(BUS_NAME, bus=config.session_bus, replace_existing=False,
                                        allow_replacement=False, do_not_queue=True)
...
class G15GlobalConfig:
    def __init__(self, parent, widget_tree, conf_client):
        ...
        only_show_indicator_on_error.set_visible(
            g15desktop.is_desktop_application_installed("g15-indicator") and not gnome_shell)
        start_indicator_on_login.set_visible(
            g15desktop.is_desktop_application_installed("g15-indicator") and not gnome_shell)
        start_system_tray_on_login.set_visible(
            g15desktop.is_desktop_application_installed("g15-systemtray") and not gnome_shell)
        ...
        enable_gnome_shell_extension.set_active(
            g15desktop.is_gnome_shell_extension_enabled("gnome15-shell-extension@gnome15.org"))
    ...
    def run(self):
        ...
        if len(self.global_plugin_model) == 0:
            ...
            self.global_plugin_tree.get_selection().select_path(
                self.global_plugin_model.get_path(self.global_plugin_model.get_iter(0)))
            self._select_plugin()
    ...
    def _select_plugin(self, widget=None):
        ...
        if plugin != None:
            ...
            self.widget_tree.get_object("SupportedLabel").set_text(
                ", ".join(g15pluginmanager.get_supported_models(plugin)).upper())
        ...
        for mod in sorted(g15pluginmanager.imported_plugins, key=lambda key: key.name):
            if g15pluginmanager.is_global_plugin(mod):
                ...
                if mod.id == self.selected_id:
                    self.global_plugin_tree.get_selection().select_path(self.global_plugin_model.get_path(
                        self.global_plugin_model.get_iter(len(self.global_plugin_model) - 1)))
...
class G15Config:
    ...
    def __init__(self, parent_window=None, service=None, options=None):
        ...
        g15uigconf.configure_checkbox_from_gconf(self.conf_client, "/apps/gnome15/indicate_only_on_error",
                                                 "OnlyShowIndicatorOnError", False, self.widget_tree, True)
    ...
    def _connect(self):
        ...
        # Watch for changes
        self._signal_handles.append(
            self.session_bus.add_signal_receiver(self._starting, dbus_interface="org.gnome15.Service",
                                                 signal_name='Starting'))
        self._signal_handles.append(
            self.session_bus.add_signal_receiver(self._started, dbus_interface="org.gnome15.Service",
                                                 signal_name='Started'))
        self._signal_handles.append(
            self.session_bus.add_signal_receiver(self._stopping, dbus_interface="org.gnome15.Service",
                                                 signal_name='Stopping'))
        self._signal_handles.append(
            self.session_bus.add_signal_receiver(self._stopped, dbus_interface="org.gnome15.Service",
                                                 signal_name='Stopped'))
        self._signal_handles.append(
            self.session_bus.add_signal_receiver(self._screen_added, dbus_interface="org.gnome15.Service",
                                                 signal_name='ScreenAdded'))
        self._signal_handles.append(
            self.session_bus.add_signal_receiver(self._screen_removed, dbus_interface="org.gnome15.Service",
                                                 signal_name='ScreenRemoved'))
        self._signal_handles.append(
            self.session_bus.add_signal_receiver(self._status_change, dbus_interface="org.gnome15.Screen",
                                                 signal_name='Connected'))
        self._signal_handles.append(
            self.session_bus.add_signal_receiver(self._status_change, dbus_interface="org.gnome15.Screen",
                                                 signal_name='ConnectionFailed'))
        self._signal_handles.append(
            self.session_bus.add_signal_receiver(self._status_change, dbus_interface="org.gnome15.Screen",
        ...
        if not self.gnome15_service or self.state == STOPPED:
            ...
            if connected != screen_count and first_error is not None and first_error != "":
                if len(self.screen_services) == 1:
                    ...
                    mesg = (
                           "The Gnome15 desktop service is running, but only %d out of %d keyboards are connected. The first error message given was %s") % (
    ...
    def _show_preferences(self, widget):
        plugin = self._get_selected_plugin()
        plugin.show_preferences(self.main_window, self.driver, self.conf_client,
                                self._get_full_key("plugins/%s" % plugin.id))
    ...
    def _load_plugins(self):
        ...
        if self.selected_device:
            ...
            for mod in sorted(g15pluginmanager.imported_plugins, key=lambda key: key.name):
                ...
                if self.driver and self.driver.get_model_name() in g15pluginmanager.get_supported_models(
                        mod) and not g15pluginmanager.is_global_plugin(mod):
                    enabled = self.conf_client.get_bool(key)
                    self.plugin_model.append([enabled, mod.name, mod.id])
                    if mod.id == self.selected_id:
                        self.plugin_tree.get_selection().select_path(
                            self.plugin_model.get_path(self.plugin_model.get_iter(len(self.plugin_model) - 1)))
    ...
    def _select_plugin(self, widget):
        ...
        if plugin != None:
            ...
            self.widget_tree.get_object("SiteLabel").set_uri(plugin.site)
            self.widget_tree.get_object("SiteLabel").set_label(plugin.site)
            self.widget_tree.get_object("PreferencesButton").set_sensitive(
                plugin.has_preferences and self.driver is not None)
        ...
        if active_profile is None:
            logger.warning(
                "No active profile found. It's possible the profile no longer exists, or is supplied with a plugin that cannot be found.")
        else:
            bindings = []
    ...
    def _set_enabled_value_from_configuration(self):
        enabled = g15devices.is_enabled(self.conf_client,
                                        self.selected_device) if self.selected_device != None else False
    ...
    def _load_device(self):
        ...
        if self.selected_device != None:
            ...
            self.notify_handles.append(self.conf_client.notify_add(self._get_full_key("cycle_screens"),
                                                                   self._cycle_screens_configuration_changed));
            self.notify_handles.append(
                self.conf_client.notify_add(self._get_full_key("plugins"), self._plugins_changed))
            self.notify_handles.append(
                self.conf_client.notify_add(self._get_full_key("active_profile"), self._active_profile_changed))
            self.notify_handles.append(
                self.conf_client.notify_add(self._get_full_key("locked"), self._active_profile_changed))
            self.notify_handles.append(
                self.conf_client.notify_add(self._get_full_key("enabled"), self._device_enabled_configuration_changed))
            self.notify_handles.append(
                self.conf_client.notify_add(self._get_full_key("driver"), self._driver_configuration_changed))
    ...
    def _set_available_profile_actions(self):
        ...
        self.enabled_profile_plugins.set_sensitive(
            not self.selected_profile.read_only and self.profile_plugins_mode_model[path][
                0] == g15profile.SELECTED_PLUGINS)
        self.window_name.set_sensitive(not self.selected_profile.read_only and self.selected_profile.activate_on_focus)
        self.window_select.set_sensitive(
            not self.selected_profile.read_only and self.selected_profile.activate_on_focus)
        ...
        if response == gtk.RESPONSE_OK:
            ...
            try:
                ...
                if everything_ok:
                    ...
                    if everything_ok:
                        ...
                        if imported_profile.icon:
                            imported_profile.icon = "%s/%d.%s" % (
                            profile_dir, profile_id, imported_profile.icon.split(".", 1)[1] )
                        if imported_profile.background:
                            imported_profile.background = "%s/%d.%s" % (
                            profile_dir, profile_id, imported_profile.background.split(".", 1)[1] )
                ...
                if not everything_ok:
                    ...
                    import_profile_error_dialog_close_button = self.widget_tree.get_object(
                        "ImportProfileErrorCloseButton")
                    import_profile_error_dialog_close_button.connect("clicked",
                                                                     lambda x: import_profile_error_dialog.hide())
        ...
        for device in self.devices:
            if device.model_id == 'virtual':
                icon_file = g15icontools.get_icon_path(
                    ["preferences-system-window", "preferences-system-windows", "gnome-window-manager",
                     "window_fullscreen"])
            else:
        ...
        if self.selected_device != None:
            ...
            for profile in self.profiles:
                ...
                if current_selection != None and profile.id == current_selection.id:
                    tree_selection.select_path(
                        self.profiles_model.get_path(self.profiles_model.get_iter(len(self.profiles_model) - 1)))
        ...
        try:
            ...
            for macro in macros:
                ...
                if current_selection != None and macro.key_list_key == current_selection.key_list_key:
                    tree_selection.select_path(
                        self.macros_model.get_path(self.macros_model.get_iter(len(self.macros_model) - 1)))
        ...
        if self.selected_device:
            for mod in sorted(g15pluginmanager.imported_plugins, key=lambda key: key.name):
                ...
                if self.driver and self.driver.get_model_name() in g15pluginmanager.get_supported_models(
                    ...
                    enabled = self.conf_client.get_bool(key)
                    if enabled:
                        self.enabled_profile_plugins_model.append(
                            [mod.id in self.selected_profile.selected_plugins, mod.name, mod.id])
