...
class G15MacroEditor():
        ...
        try:
            ...
            for index, row in enumerate(self.__map_type_model):
                if row[0] == self.editing_macro.type:
                    ...
                    break
            self.__load_keys()
            if self.editing_macro.type in [g15profile.MACRO_MOUSE, g15profile.MACRO_JOYSTICK,
                                           g15profile.MACRO_DIGITAL_JOYSTICK, g15profile.MACRO_KEYBOARD]:
    ...
    def __set_available_options(self):
        ...
        self.__turbo_box.set_sensitive(
            self.editing_macro.repeat_mode != g15profile.NO_REPEAT and self.__override_default_repeat.get_active())
...
class G15MacroScriptEditor():
    def __init__(self, gconf_client, driver, editing_macro, parent=None):
        ...
        self.__output_delays.set_active(
            g15gconf.get_bool_or_default(self.__gconf_client, "/apps/gnome15/script_editor/record_delays", True))
        self.__emit_uinput.set_active(
            g15gconf.get_bool_or_default(self.__gconf_client, "/apps/gnome15/script_editor/emit_uinput", False))
    ...
    def _on_new_goto(self, widget):
        ...
        if response == gtk.RESPONSE_OK:
            self._insert_macro(
                "%s %s" % ( self._format_op("goto"), self.__goto_label_model[self.__goto_label.get_active()][0]))
    ...
    def _on_new_wait(self, widget):
        ...
        if response == gtk.RESPONSE_OK:
            self._insert_macro(
                "%s %s" % ( self._format_op("wait"), self.__wait_model[self.__wait_combo.get_active()][0]))
    ...
    def _set_available(self):
        ...
        self.__record_status.set_text(_("Now recording (%d) operations" % ops) if self.__recorder.is_recording() else (
        _("Will insert %d operations" % ops) if ops > 0 else ""))
        ...
        for idx, row in enumerate(self.__script_model):
            ...
            if isinstance(show_ops, list) and op in show_ops or op == show_ops:
                tree_selection.select_path(
                    self.__script_model.get_path(self.__script_model.get_iter_from_string("%d" % idx)))

    def _select_row(self, row):
        self.__script_tree.get_selection().select_path(
            self.__script_model.get_path(self.__script_model.get_iter_from_string("%d" % row)))
