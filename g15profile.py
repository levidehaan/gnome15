...
class G15Macro(object):
    ...
    def _get(self, key, default_value):
        ...
        return self.profile.parser.get(section_name, option_key) if self.profile.parser.has_option(section_name,
...
class G15Profile(object):
    ...
    def are_keys_in_use(self, activate_on, memory, keys, exclude=None):
        ...
        for macro in bank:
            if ( exclude == None or ( exclude != None and not self._is_excluded(exclude, macro) ) ) and sorted(
                    keys) == sorted(macro.keys):
                return True
        return False
    ...
    def save(self, filename=None):
        ...
        if self.version == 1.0:
            self.parser.set("DEFAULT", "base_profile",
                            str(self.base_profile) if self.base_profile is not None else "-1")
        else:
        ...
        for activate_on in [g15driver.KEY_STATE_UP, g15driver.KEY_STATE_DOWN, g15driver.KEY_STATE_HELD]:
            for i in range(1, 4):
                ...
                self.mkey_color[i] = g15convert.to_rgb(
                    self.parser.get(section_name, "backlight_color")) if self.parser.has_option(section_name,
                                                                                                "backlight_color") else None
                memory_macros = []
