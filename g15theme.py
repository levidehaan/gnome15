...
class Component(object):
    ...
    def add_child(self, child, index=-1):
        ...
        try:
            if child.parent:
                raise Exception(
                    "Child %s already has a parent. Remove it from it's last parent first before adding to %s." % (
                    child.id, self.id))
            if child.id in self.child_map:
                raise Exception(
                    "Child with ID of %s already exists in component %s. Trying to add %s, but %s exists" % (
                    child.id, self.id, str(child), str(self.child_map[child.id])))
...
class G15Page(Component):
    def __init__(self, page_id, screen, painter=None, priority=g15screen.PRI_NORMAL, on_shown=None, on_hidden=None,
                 on_deleted=None, \
    ...
    def arc(self, x, y, radius, angle1, angle2, fill=False):
        self.back_context.arc(x, y, radius, g15convert.degrees_to_radians(angle1),
                              g15convert.degrees_to_radians(angle2))
        if fill:
    ...
    def check_scroll_and_reschedule(self):
        ...
        try:
            ...
            if scroll and self.theme_scroll_timer == None:
                self.theme_scroll_timer = g15scheduler.schedule("ScrollRedraw", self.screen.service.scroll_delay,
                                                                self.scroll_and_reschedule)
    ...
    def paint(self, canvas):
        ...
        try:
            ...
            if new_base != self.base:
                ...
                if self.get_screen().service.animated_menus:
                    self.scroll_timer = g15scheduler.schedule("ScrollTo", self.get_screen().service.animation_delay,
                                                              self.get_root().redraw)
                else:
                    self.get_root().redraw()
...
class MenuItem(Component):
    def __init__(self, component_id="menu-entry", group=True, name=None, alt="", activate=None, icon=None,
                 activatable=True):
        Component.__init__(self, component_id)
        self.group = group
...
class DBusMenuItem(MenuItem):
    ...
    def get_theme_properties(self):
        ...
        if icon_name != None:
            properties["item_icon"] = g15cairo.load_surface_from_file(g15icontools.get_icon_path(icon_name),
                                                                      self.theme.bounds[3])
        else:
...
class G15Theme(object):
        ...
        try:
            ...
            if self.page is None:
                ...
                if self.dir != None:
                    ...
                    prefix_path = self.prefix if self.prefix != None else os.path.basename(
                        os.path.dirname(self.dir)).replace("-", "_") + "_" + self.theme_name + "_"

                    # The theme may have a python portion
                    module_name = self.get_path_for_variant(self.dir, self.variant, "py", fatal=False,
                                                            prefix=prefix_path)
                    module = None
                    if module_name != None:
    ...
    def del_namespace(self, prefix, uri):
        for e in self.document.getroot().xpath("//*[namespace-uri()='%s' or @*[namespace-uri()='%s']]" % ( uri, uri ),
    ...
    def get_path_for_variant(self, dir, variant, extension, fatal=True, prefix=""):
        ...
        if not os.path.exists(path):
            ...
            path = os.path.join(dir, g15globals.themes_dir, "default",
                                self.driver.get_model_name() + variant + "." + extension)
            if not os.path.exists(path):
                ...
                if not os.path.exists(path):
                    ...
                    if not os.path.exists(path):
                        if fatal:
                            raise Exception("Missing %s. No .%s file for model %s in %s for variant %s" % (
                            path, extension, self.driver.get_model_name(), dir, variant ))
                        else:
                            return None
        return path
    ...
    def draw(self, canvas, properties={}, attributes={}):
        ...
        if self.render == None or self.dirty:
            ...
            try:
                ...
                self._do_shadow("shadow",
                                self.screen.driver.get_color_as_hexrgb(g15driver.HINT_BACKGROUND, (255, 255, 255)),
                                root)
                self._do_shadow("reverseshadow",
                                self.screen.driver.get_color_as_hexrgb(g15driver.HINT_FOREGROUND, (0, 0, 0)), root)
            ...
            if title != None:
                ...
                if args[0] == "del":
                    ...
                    if ( condition and var in properties and properties[var] != "" and properties[var] != False ) or \
                            (not condition and (
                                    not var in properties or properties[var] == "" or properties[var] == False ) ):
                        element.getparent().remove(element)
            ...
            if id.endswith("_progress"):
                ...
                if property_key in properties:
                    ...
                    logger.warning("Found progress element with an ID that doesn't exist in " + \
                                   "theme properties. Theme directory is %s, variant is %s." % (
                                   self.dir, self.variant ))
            else:
    ...
    def _set_highlight_color(self, root):
        ...
        if self.screen.driver.get_control_for_hint(g15driver.HINT_HIGHLIGHT):
            for element in root.xpath('//svg:*[@style]', namespaces=self.nsmap):
                element.set("style", element.get("style").replace(DEFAULT_HIGHLIGHT_COLOR,
                                                                  self.screen.driver.get_color_as_hexrgb(
                                                                      g15driver.HINT_HIGHLIGHT, (255, 0, 0 ))))
    ...
    def _set_relative_image_paths(self, root):
        for element in root.xpath('//svg:image[@xlink:href]', namespaces=self.nsmap):
            ...
            is_abs = href and (
            href.startswith("http:") or href.startswith("https:") or href.startswith("file:") or href.startswith("/"))
    ...
    def _get_clip_path_element(self, element):
        ...
        if clip_val and len(clip_val) > 0 and clip_val != "none":
            ...
            if el is None:
                raise Exception(
                    "Text node had clip path (%s), but no clip path element with matching ID of %s could be found" % (
                    id, element.get("clip-path") ))
            return el
