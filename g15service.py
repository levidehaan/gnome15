...
if g15desktop.get_desktop() in ["gnome", "gnome-shell"]:
    try:
        ...
        logger.debug("Could not get gnome master client %s" % str(e))
        pass
...
class G15Service(g15desktop.G15AbstractService):
    ...
    def _monitor_session(self):
        ...
        if self.system_bus.name_has_owner('org.freedesktop.ConsoleKit'):
            ...
            logger.warning(
                "None of the supported system session manager available, will not track active desktop session.")
            self.session_active = True
        ...
        try:
            ...
            session_manager_object = self.session_bus.get_object("org.gnome.SessionManager",
                                                                 "/org/gnome/SessionManager",
                                                                 "org.gnome.SessionManager")
            client_path = session_manager_object.RegisterClient('gnome15.desktop', '',
                                                                dbus_interface="org.gnome.SessionManager")

            self.session_manager_client_object = self.session_bus.get_object("org.gnome.SessionManager", client_path,
        ...
        if not connected_to_session_manager:
            try:
                ...
                session_manager_object = self.session_bus.get_object("org.mate.SessionManager",
                ...
                session_manager_client_public_object = self.session_bus.get_object("org.mate.SessionManager",
                                                                                   client_path,
        ...
        if not connected_to_session_manager:
            logger.warning(
                "None of the supported session managers available, will not detect logout signal for clean shutdown.")
    ...
    def _connect_to_consolekit(self):
        try:
            ...
            self.system_bus.add_signal_receiver(self._active_session_changed,
                                                dbus_interface="org.freedesktop.ConsoleKit.Seat",
                                                signal_name="ActiveSessionChanged")
            console_kit_object = self.system_bus.get_object("org.freedesktop.ConsoleKit",
    ...
    def _connect_to_logind(self):
        try:
            ...
            self.system_bus.add_signal_receiver(self._logind_seat0_property_changed, "PropertiesChanged",
                                                "org.freedesktop.DBus.Properties", "org.freedesktop.login1",
    ...
    def _configure_window_monitoring(self):
        ...
        try:
            ...
            self.session_bus.add_signal_receiver(self._active_window_changed, dbus_interface='org.ayatana.bamf.matcher',
    ...
    def _device_added(self, device):
        ...
        self.device_notify_handles[device.uid] = self.conf_client.notify_add("/apps/gnome15/%s/enabled" % device.uid,
                                                                             self._device_enabled_configuration_changed,
                                                                             device)
