...
controls = {
    ...
    g15driver.MODEL_G15_V1: [mkeys_control, backlight_control, lcd_contrast_control, lcd_backlight_control,
                             invert_control],
...
class Driver(g15driver.AbstractDriver):
    ...
    def _on_connect(self):
        ...
        self.notify_handles.append(
            self.conf_client.notify_add("/apps/gnome15/%s/timeout" % self.device.uid, self._config_changed, None))
        self.notify_handles.append(
            self.conf_client.notify_add("/apps/gnome15/%s/digital_offset" % self.device.uid, self._config_changed,
                                        None))
        self.notify_handles.append(
            self.conf_client.notify_add("/apps/gnome15/%s/analogue_offset" % self.device.uid, self._config_changed,
                                        None))
    ...
    def _load_configuration(self):
        ...
        self.digital_calibration = g15gconf.get_int_or_default(self.conf_client,
                                                               "/apps/gnome15/%s/digital_offset" % self.device.uid, 63)
        self.analogue_calibration = g15gconf.get_int_or_default(self.conf_client,
                                                                "/apps/gnome15/%s/analogue_offset" % self.device.uid,
                                                                20)
    ...
    def _handle_key_event(self, code, ext_code):
        ...
        if self.get_model_name() == g15driver.MODEL_G13:
            c = self.analogue_calibration if self.joy_mode in [g15uinput.JOYSTICK,
                                                               g15uinput.MOUSE] else self.digital_calibration
