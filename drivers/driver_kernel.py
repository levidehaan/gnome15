...
g19_keyboard_backlight_control = g15driver.Control("backlight_colour", _("Keyboard Backlight Colour"), (0, 255, 0),
                                                   hint=g15driver.HINT_DIMMABLE | g15driver.HINT_SHADEABLE)

g19_brightness_control = g15driver.Control("lcd_brightness", _("LCD Brightness"), 100, 0, 100,
                                           hint=g15driver.HINT_SHADEABLE)
g19_foreground_control = g15driver.Control("foreground", _("Default LCD Foreground"), (255, 255, 255),
                                           hint=g15driver.HINT_FOREGROUND | g15driver.HINT_VIRTUAL)
g19_background_control = g15driver.Control("background", _("Default LCD Background"), (0, 0, 0),
                                           hint=g15driver.HINT_BACKGROUND | g15driver.HINT_VIRTUAL)
g19_highlight_control = g15driver.Control("highlight", _("Default Highlight Color"), (255, 0, 0),
                                          hint=g15driver.HINT_HIGHLIGHT | g15driver.HINT_VIRTUAL)
g19_controls = [g19_brightness_control, g19_keyboard_backlight_control, g19_foreground_control, g19_background_control,
                g19_highlight_control, g19_mkeys_control]

g110_keyboard_backlight_control = g15driver.Control("backlight_colour", _("Keyboard Backlight Colour"), (255, 0, 0),
...
g15_backlight_control = g15driver.Control("keyboard_backlight", _("Keyboard Backlight Level"), 2, 0, 2,
                                          hint=g15driver.HINT_DIMMABLE)
...
g15_invert_control = g15driver.Control("invert_lcd", _("Invert LCD"), 0, 0, 1,
                                       hint=g15driver.HINT_SWITCH | g15driver.HINT_VIRTUAL)
g15_controls = [g15_mkeys_control, g15_backlight_control, g15_invert_control, g15_lcd_backlight_control,
...
class KernelDriverPreferences():
    def __init__(self, device, parent, gconf_client):
        ...
        g15uigconf.configure_combo_from_gconf(gconf_client, "/apps/gnome15/%s/fb_device" % device.uid, "DeviceCombo",
                                              "auto", widget_tree)
        g15uigconf.configure_combo_from_gconf(gconf_client, "/apps/gnome15/%s/joymode" % device.uid, "JoyModeCombo",
                                              "macro", widget_tree)
...
class Driver(g15driver.AbstractDriver):
    def __init__(self, device, on_close=None):
        ...
        self.notify_handles.append(
            self.conf_client.notify_add("/apps/gnome15/%s/joymode" % self.device.uid, self._config_changed, None))
        self.notify_handles.append(self.conf_client.notify_add("/apps/gnome15/%s/fb_device" % self.device.uid,
                                                               self._framebuffer_device_changed, None))
        self.notify_handles.append(
    ...
    def process_svg(self, document):
        if self.get_bpp() == 1:
            for element in document.getroot().iter():
                ...
                if style != None:
                    element.set("style",
                                style.replace("font-family:Sans", "font-family:%s" % g15globals.fixed_size_font_name))
...
It should be launched automatically if Gnome15 is installed correctly.")
    ...
    def _init_driver(self):
        ...
        for p in os.listdir(dir):
            ...
            if self.grab_multimedia and self.device_info.mm_pattern is not None and re.search(
                    self.device_info.mm_pattern, p):
