...
class G15Screen():
        ...
        if self.defeat_profile_change < 1 and not g15profile.is_locked(self.device):
            ...
            if application_name is not None:
                for profile in g15profile.get_profiles(self.device):
                    if not profile.get_default() and profile.activate_on_focus and len(
                            profile.window_name) > 0 and application_name.lower().find(
                            profile.window_name.lower()) != -1:
            ...
            if choose_profile == None:
                ...
                if (
                        active_profile == None or active_profile.id != default_profile.id) and default_profile.activate_on_focus:
    ...
    def start(self):
        ...
        for control in self.driver.get_controls():
            self.notify_handles.append(
                self.conf_client.notify_add("%s/%s" % (screen_key, control.id), self._control_changed))
        ...
        if self.is_active() and not quickly and (self.service.fade_screen_on_close \
            ...
            if self.service.fade_keyboard_backlight_on_close:
                ...
                if bl_control:
                    ...
                    acquisition.fade(duration=slow_shutdown_duration, release=True,
                                     step=1 if isinstance(current_val, int) else 10)
    ...
    def resched_cycle(self, arg1=None, arg2=None, arg3=None, arg4=None):
        ...
        try:
            ...
            self._cancel_timer()
            cycle_screens = g15gconf.get_bool_or_default(self.conf_client,
                                                         "/apps/gnome15/%s/cycle_screens" % self.device.uid, True)
    ...
    def cycle_color(self, val, control):
        ...
        self.conf_client.set_string("/apps/gnome15/%s/%s" % (self.device.uid, control.id),
